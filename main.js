...
//let projD = canvas.height / (2*Math.tan(projA/2)); // vertical FOV
  ...
  let objects = [
    createSphere([0.0,2.5,-2.0],0.5,createMaterial([0.5,0.5,0.5],0.5,1.0,500.0,0.8)),
    ...
    createSphere([ 0.0,0.5,2.0],0.5,createMaterial([1.0,1.0,1.0],1.0,0.3,5.0,0.0)),
...
*/
  return [
    Math.min(1, rgb[0] + ref[0]),
    Math.min(1, rgb[1] + ref[1]),
    Math.min(1, rgb[2] + ref[2]),
  ];
}
